...
def test(argv=sys.argv):
    ...
    series of lat lon points on Europa, over the course of most of an orbit,
    and also at a variety of different amounts of viscous relaxation.  Compares
    the calculated values to those listed in the C{pickle} file passed in via
    the command line.

    C{test} is called from the C{SatStress Makefile}, when one does C{make
    test}, with the appropriate C{pickle}d input to compare against (it is
    ...
    @type argv: list
    @return: C{True} if the test fails. C{False} if the test passes.
    @rtype: bool
    """
    ...
    the_sat = Satellite(open(args[0],'r'))
...
# This allows the module to run as a standalone script
